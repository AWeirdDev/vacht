...
class VachtNotFound(FileNotFoundError): ...


def find_vacht_bin() -> str:
    """Return the vacht binary path."""

    vacht_exe = "vacht" + sysconfig.get_config_var("EXE")
    ...
    targets = [
        ...
        # Above the package root, e.g., from `pip install --prefix` or `vacht run --with`
        (
            # On Windows, with module path `<prefix>/Lib/site-packages/vacht`
            _join(
                _matching_parents(_module_path(), "Lib/site-packages/vacht"), "Scripts"
            )
            if sys.platform == "win32"
            # On Unix,  with module path `<prefix>/lib/python3.13/site-packages/vacht`
            else _join(
                _matching_parents(_module_path(), "lib/python*/site-packages/vacht"),
                "bin",
            )
        ),
        # Adjacent to the package root, e.g., from `pip install --target`
        # with module path `<target>/vacht`
        _join(_matching_parents(_module_path(), "vacht"), "bin"),
    ...
    for target in targets:
        ...
        if target in seen:
            continue
        seen.append(target)
        path = os.path.join(target, vacht_exe)
        if os.path.isfile(path):
            return path

    locations = "\n".join(f" - {target}" for target in seen)
    raise VachtNotFound(
        f"Could not find the vacht binary in any of the following locations:\n{locations}\n"
    )
